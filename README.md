...
1. **Google Places API** (Primary) - Searches for "skateboard shop" across 220+ US metro areas using the Text Search API with pagination (up to 60 results per metro). Returns both independent and chain stores. Requires `GOOGLE_PLACES_API_KEY` environment variable. Free tier: 1,000 requests/month (we use ~220-660).
